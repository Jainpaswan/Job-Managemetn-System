...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const userEmail = decoded.sub || decoded.email || email; // depends on backend claims

      dispatch(setCredentials({ token, user: userEmail }));
      // dispatch(setToken(res.data.token)); // save in Redux + localStorage
      navigate("/Home");
    } catch (err) {
      console.error(err);
      alert("Login failed!");
  ...
  <svg
    ...
    strokeWidth={1.5}
    stroke="currentColor"
    className="w-6 h-6"  // ✅ width and height
  >
    <path
      strokeLinecap="round"
